...
More developed levels may be autogeneration of SQL and using prepared in database metadata.
...
Object data: 
C<-wgtbl>, C<-wgbln>, C<-wgarr>, C<-wgnav>, C<-wgind>,
C<-mdedt>,
C<-dsdtm>,
C<-cbcmd>, C<-cbXXX>,
C<-dbh>, C<-dbfnm>, C<-dbfds>,
C<-parm>
...
C<$do-E<gt>DBICmd>, C<$do-E<gt>DBIHlp>
...
This is simplest C<DBI> interface, it executes C<dbh> command.
...
=item "12/08/2000" - fixed bad event type or keysym 'ô'
