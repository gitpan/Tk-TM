...
C<-wgmnu>, C<-wglst>, C<-wgscr>, C<-title>,
C<-mdlst>,
C<-parm>
...
Default is 'tree', that means that detail screen should be opened
...
=item setscr (level, title, ?sub, ?opt)
...
Opt is options (scalar or ref), used within sub.
Special title 'Login' is used for predefined login screen.
...
Label of the screen as displayed in C<-wglst>.
...
This is the third part of application window title. Other
parts are the title of the application and the label of the screen C<-label>.


=item -wgapp

Application window, provided by C<Tk::wApp>.


=item -wgmnu

Application`s menu bar, provided by C<Tk::wApp>.


=item -wgscr

Widget for application screen, to create application widgets inside it.
Provided by C<Tk::wApp>.
C<Tk::Frame> widget is used.
